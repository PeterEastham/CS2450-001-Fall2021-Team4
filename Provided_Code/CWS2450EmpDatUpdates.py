...
this puts the entry widget in the space of a resizable tk frame
of any size.
...
trys and catches may be a better choice(WHAT???). There are pleny of examples on the internet
...
import tkinter as tk #There has to be a better way
...
class FiveCharEntry2(ttk.Entry):
    def __init__(self, parent, *args, **kwargs): #Sloppy
        super().__init__(parent, *args, **kwargs) #Even sloppier
    ...
    def _validate(self,proposed_value):
        return len(proposed_value)<=5 #OOP but functional outcome?

    #delete all keys past 5 keys on loss of focus
    def _on_invalid(self):
        self.delete(5,tk.END) #Invalid makes no sense. Also should delete all
